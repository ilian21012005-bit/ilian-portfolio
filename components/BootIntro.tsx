      ...
      <AnimatePresence>
        {showBoot === true && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-black flex flex-col p-6 font-mono text-sm text-accent-secondary overflow-hidden"
          >
            <div className="flex-1 overflow-auto space-y-0.5">
              {lines.map((line, i) => (
                <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-accent-secondary/90">
                  {line}
                </motion.div>
              ))}
            </div>
            <span className="inline-block w-2 h-4 bg-accent-secondary animate-pulse" />
          </motion.div>
        )}
      </AnimatePresence>
    </>
